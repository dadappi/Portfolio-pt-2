...
import { Crab, Seagull, Sun, Cloud, WaveDivider } from './Decor'
...
export default function Hero() {
  ...
  return (
    <section
      id="hero"
      className="relative min-h-screen flex items-center justify-center pt-16 overflow-hidden bg-gradient-to-b from-sky-200 via-sky-100 to-[var(--color-bg)]"
      aria-labelledby="hero-title"
    >
      {/* langit & elemen dekoratif */}
      <motion.div
        ...
        aria-hidden="true"
      >
        <Cloud className="w-28 h-16 text-white" />
      </motion.div>
      <motion.div
        ...
        aria-hidden="true"
      >
        <Cloud className="w-20 h-12 text-white" />
      </motion.div>
      <div className="absolute top-10 right-[8%]" aria-hidden="true">
        <Sun className="w-16 h-16 md:w-24 md:h-24" />
      </div>
      <motion.div
        className="absolute top-24 left-1/3 hidden md:block"
        animate={{ y: [0, -8, 0] }}
        transition={{ duration: 4, repeat: Infinity, ease: 'easeInOut' }}
        aria-hidden="true"
      >
        <Seagull className="w-12 h-12 text-text" />
      </motion.div>

      {/* kepiting nongol di pojok bawah */}
      <motion.div
        className="absolute bottom-16 left-[5%] hidden sm:block"
        animate={{ y: [0, -6, 0], rotate: [-3, 3, -3] }}
        transition={{ duration: 3, repeat: Infinity, ease: 'easeInOut' }}
        aria-hidden="true"
      >
        <Crab className="w-16 h-16 md:w-20 md:h-20" />
      </motion.div>
      ...
      <div className="section-container relative z-10 py-20">
        ...
        >
          <motion.span
            ...
            className="inline-block px-4 py-1.5 rounded-full text-sm font-medium bg-gradient-to-r from-orange-500/20 to-teal-500/20 border border-orange-500/30 text-orange-700 mb-6"
          >
              ...
              return (
                <motion.a
                  ...
                  className={`inline-flex items-center gap-2 px-6 py-3 rounded-full font-medium transition-all ${
                    btn.primary
                      ? 'bg-gradient-to-r from-orange-500 to-teal-500 text-white hover:from-orange-600 hover:to-teal-600 shadow-lg shadow-orange-500/25'
                      : 'bg-bg-elevated text-text border border-border hover:border-orange-500/50 hover:bg-orange-500/10'
                  }`}
            ...
            {SOCIAL_LINKS.map((social, i) => {
              ...
              return (
                <motion.a
                  ...
                  className="p-2 rounded-full bg-bg-elevated border border-border text-text-muted hover:text-text hover:border-orange-500/50 transition-all"
