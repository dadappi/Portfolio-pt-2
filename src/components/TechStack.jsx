      ...
      <div className="section-container">
          ...
          {techStack.map((cat, i) => (
            ...
            >
              ...
              <span className={`ml-2 px-2 py-0.5 text-xs rounded-full ${
                activeCategory === cat.category ? 'bg-white/20' : 'bg-orange-500/10'
              }`}>
                {cat.tools?.length || 0}
              </span>
            </motion.button>
          ...
          >
            {currentCategory?.tools.map((tool, i) => (
              <motion.article
                ...
                className="group bg-bg-elevated border border-border rounded-2xl p-6 hover:border-orange-500/50 transition-all"
              >
                <div className="flex items-start justify-between gap-4 mb-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-medium text-orange-500 whitespace-nowrap">{tool.level}%</span>
                    </div>
                  </div>
                  <motion.button
                    onClick={() => toggleExpand(tool.name)}
                    className="p-2 rounded-lg bg-border text-text-muted hover:bg-orange-500/10 hover:text-orange-500 transition-colors flex-shrink-0"
                    whileTap={{ scale: 0.9 }}
        ...
        >
          ...
          <div className="flex justify-center">
            <motion.a
              href="#contact"
              className="inline-flex items-center gap-2 px-8 py-3 bg-gradient-to-r from-orange-500 to-teal-500 text-white rounded-full font-medium hover:from-orange-600 hover:to-teal-600 transition-all shadow-lg shadow-orange-500/25"
              whileHover={{ y: -2, scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
            >
              Hubungi Saya
