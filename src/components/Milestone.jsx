    ...
    >
      <div className="section-container">
        ...
        <div className="relative max-w-3xl mx-auto">
          <motion.div
            className="absolute left-1/2 top-0 bottom-0 w-0.5 -translate-x-1/2 bg-gradient-to-b from-orange-500 to-teal-500"
            initial={{ scaleY: 0 }}
          ...
          {milestones.map((milestone, index) => {
            ...
            return (
              ...
              >
                <div className={`absolute top-4 ${isEven ? 'right-0' : 'left-0'} translate-x-1/2 w-10 h-10 rounded-full bg-gradient-to-br from-orange-500 to-teal-500 flex items-center justify-center text-white z-10 shadow-lg shadow-orange-500/25`}>
                  <Icon className="w-5 h-5" aria-hidden="true" />
                </div>

                <motion.div
                  className={`bg-bg-elevated border border-border rounded-2xl p-6 ${isEven ? 'mr-4' : 'ml-4'} hover:border-orange-500/50 transition-all`}
                  whileHover={{ x: isEven ? -4 : 4 }}
                >
                  <div className="flex items-center justify-between gap-4 mb-3">
                    <span className="px-3 py-1 bg-orange-500/10 text-orange-500 text-sm font-bold rounded-full whitespace-nowrap">
                      {milestone.year}
                    </span>
                  </div>
