    ...
    >
      <div className="section-container">
        ...
        <div className="relative max-w-3xl mx-auto">
          <motion.div
            className="absolute left-8 top-0 bottom-0 w-0.5 bg-gradient-to-b from-orange-500 to-teal-500"
            initial={{ scaleY: 0 }}
          ...
          {experience.map((exp, index) => (
            ...
            >
              <motion.div
                className="absolute left-0 top-1 w-4 h-4 rounded-full bg-gradient-to-r from-orange-500 to-teal-500 border-4 border-bg z-10"
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
              ...
              <motion.div
                className="bg-bg-elevated border border-border rounded-2xl p-6 hover:border-orange-500/50 transition-all"
                whileHover={{ x: 4 }}
              >
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-4">
                  <div>
                    <h3 className="font-heading text-xl font-bold mb-1">{exp.role}</h3>
                    <p className="text-orange-500 font-medium">{exp.institution}</p>
                  </div>
                  <span className="px-3 py-1 bg-orange-500/10 text-orange-500 text-sm rounded-full font-medium whitespace-nowrap">
                    {exp.period}
                  </span>
                </div>
                  ...
                  <div className="flex flex-wrap gap-2 mb-5">
                    ...
                    {(exp.techStack || []).map((tech, i) => (
                      <motion.span
                        ...
                        className="px-2.5 py-1 text-xs bg-orange-500/10 text-orange-500 rounded border border-orange-500/20"
                      >
                        {tech}
                      </motion.span>
                    ))}
                  ...
                  <div className="flex flex-wrap gap-3 pt-4 border-t border-border">
                    {(exp.links || []).map((link, i) => (
                      <motion.a
                        ...
                        className="inline-flex items-center gap-1.5 text-sm text-text-muted hover:text-orange-500 transition-colors"
                        whileHover={{ x: 2 }}
                      >
